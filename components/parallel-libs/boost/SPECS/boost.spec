...
Source0:        boost_%{version_exp}.tar.gz
...
BuildRequires:  xz-%{compiler_family}%{UARCH_DELIM}%{PROJ_DELIM}

Requires:  zlib-%{compiler_family}%{UARCH_DELIM}%{PROJ_DELIM}
Requires:  bzip2-%{compiler_family}%{UARCH_DELIM}%{PROJ_DELIM}
Requires:  xz-%{compiler_family}%{UARCH_DELIM}%{PROJ_DELIM}
...
cp /opt/hpe/hpc/hmpt/hmpt-2.20/bin/mpicc  %{buildroot}/tmp
cp /opt/hpe/hpc/hmpt/hmpt-2.20/bin/mpicxx %{buildroot}/tmp
cp /opt/hpe/hpc/hmpt/hmpt-2.20/bin/mpif08 %{buildroot}/tmp
cp /opt/hpe/hpc/hmpt/hmpt-2.20/bin/mpif77 %{buildroot}/tmp
cp /opt/hpe/hpc/hmpt/hmpt-2.20/bin/mpif90 %{buildroot}/tmp

sed -i 's/\/usr\/lib64\/libcpuset\.so\.1 \/usr\/lib64\/libbitmask\.so\.1/-lcpuset -lbitmask/g' %{buildroot}/tmp/mpicc
sed -i 's/\/usr\/lib64\/libcpuset\.so\.1 \/usr\/lib64\/libbitmask\.so\.1/-lcpuset -lbitmask/g' %{buildroot}/tmp/mpicxx
sed -i 's/\/usr\/lib64\/libcpuset\.so\.1 \/usr\/lib64\/libbitmask\.so\.1/-lcpuset -lbitmask/g' %{buildroot}/tmp/mpif08
sed -i 's/\/usr\/lib64\/libcpuset\.so\.1 \/usr\/lib64\/libbitmask\.so\.1/-lcpuset -lbitmask/g' %{buildroot}/tmp/mpif77
sed -i 's/\/usr\/lib64\/libcpuset\.so\.1 \/usr\/lib64\/libbitmask\.so\.1/-lcpuset -lbitmask/g' %{buildroot}/tmp/mpif90

export MPICC=%{buildroot}/tmp/mpicc
export MPICXX=%{buildroot}/tmp/mpicxx
export MPIF77=%{buildroot}/tmp/mpif77
export MPIFC=%{buildroot}/tmp/mpif08
%endif
...
./b2 %{?_smp_mflags} -d+2 cxxflags="$CXXFLAGS" cflags="$CFLAGS" threading=multi link=static,shared variant=release --no-cmake-config --prefix=%{install_path} --user-config=./user-config.jam
...
./b2 %{?_smp_mflags} -d+2 install cxxflags="$CXXFLAGS" cflags="$CFLAGS" threading=multi link=static,shared --no-cmake-config --prefix=%{buildroot}/%{install_path} --user-config=./user-config.jam
