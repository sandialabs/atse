...
%define install_path %{OHPC_ADMIN}/%{pname}
...
%build
CFLAGS="%{optflags}" ./configure --prefix=%{install_path} --disable-pmi-backward-compatibility --enable-static || { cat config.log && exit 1; }
%{__make} %{?_smp_mflags}
